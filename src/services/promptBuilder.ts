import type { CollectedInfo } from '../types/index';
...
Bạn giao tiếp bằng ${lang.name} (${lang.nameEnglish}) thân thiện, súc tích và chuyên nghiệp.
Toàn bộ nội dung văn bản trên landing page PHẢI được viết bằng ngôn ngữ ${lang.name} (${lang.nameEnglish}).

Khi được yêu cầu tạo landing page, bạn tuân thủ nghiêm ngặt định dạng phản hồi sau:
1. Bạn có thể mở đầu bằng 2-3 câu chào và tóm tắt ý tưởng thiết kế (ngắn gọn, xúc tích, bằng ${lang.name}).
 ...
 */
export function buildGeneratePrompt(collectedInfo: CollectedInfo): string {
...
- **Ngôn ngữ nội dung:** BẮT BUỘC viết toàn bộ nội dung website bằng ${lang.name} (${lang.nameEnglish}).
...
Yêu cầu kỹ thuật:
- Mã HTML5 tự chứa (self-contained), thuộc tính <html lang="${lang.htmlLang}">, toàn bộ CSS nằm trong thẻ <style> bên trong <head>.
 ...
 */
export function buildEditPrompt(currentHTML: string, editRequest: string): string {
 ...
 * @param response - Toàn bộ chuỗi phản hồi từ AI
 * @returns Đoạn text giải thích hoặc null
 */
export function extractChatExplanation(response: string | null | undefined): string | null {
 ...
 * @param response - Toàn bộ chuỗi phản hồi từ AI
 * @returns Mã HTML sạch hoặc null
 */
export function extractHTMLFromResponse(response: string | null | undefined): string | null {
  ...
  if (match && match[1] && match[1].trim()) {
    ...
    if (
      genericMatch &&
      genericMatch[1] &&
      (genericMatch[1].includes('<!DOCTYPE') || genericMatch[1].includes('<html'))
    ) {
